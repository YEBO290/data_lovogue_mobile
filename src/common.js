...
var workspace = {
    ...
    thousandBitSeparator(num) {
      ...
      setCookie(portId, psw, exdays) {
        ...
        var cipherPortId = CryptoJS.AES.encrypt(portId+'',"currentPortId").toString()
        var cipherPsw = CryptoJS.AES.encrypt(psw+'', "pass").toString()
      ...
      getCookie: function() {
        if (document.cookie.length > 0) {
          ...
          for (var i = 0; i < arr.length; i++) {
            ...
            var bytes = CryptoJS.AES.decrypt(arr2[1], "currentPortId")
            this.currentPortId = bytes.toString(CryptoJS.enc.Utf8)
            } else if (arr2[0] == "password") {
            // Decrypt，将解密后的内容赋值给密码
            var bytes = CryptoJS.AES.decrypt(arr2[1], "pass")
